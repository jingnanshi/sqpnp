...
// helper function for calculating PnP that takes in Eigen matrices
...
public:
  ...
  inline PnPSolver(const std::vector<Point3D> &_3dpoints,
    ...
    while (7 - num_null_vectors_ >= 0 && s_[7 - num_null_vectors_] < _parameters.rank_tolerance)
      num_null_vectors_++;
...
private:
  ...
  inline static double Determinant9x1(const Eigen::Matrix<double, 9, 1> &r) {
    return r[0] * r[4] * r[8] + r[1] * r[5] * r[6] + r[2] * r[3] * r[7] - r[6] * r[4] * r[2] - r[7] * r[5] * r[0]
        - r[8] * r[3] * r[1];
  }
  ...
  inline static void NearestRotationMatrix_SVD(const Eigen::Matrix<double, 9, 1> &e, Eigen::Matrix<double, 9, 1> &r) {
    ...
    Eigen::Matrix<double, 3, 3>
        R = svd.matrixU() * Eigen::Matrix<double, 3, 1>({1, 1, detUV}).asDiagonal() * svd.matrixV().transpose();
  ...
  inline static void NearestRotationMatrix_FOAM(const Eigen::Matrix<double, 9, 1> &e, Eigen::Matrix<double, 9, 1> &r) {
    ...
    // det(B)
    detB = B[0] * B[4] * B[8] - B[0] * B[5] * B[7] - B[1] * B[3] * B[8] + B[2] * B[3] * B[7] + B[1] * B[6] * B[5]
        - B[2] * B[6] * B[4];
    ...
    // ||B||^2, ||adj(B)||^2
    Bsq = B[0] * B[0] + B[1] * B[1] + B[2] * B[2] + B[3] * B[3] + B[4] * B[4] + B[5] * B[5] + B[6] * B[6] + B[7] * B[7]
        + B[8] * B[8];
    adjBsq = adjB[0] * adjB[0] + adjB[1] * adjB[1] + adjB[2] * adjB[2] + adjB[3] * adjB[3] + adjB[4] * adjB[4]
        + adjB[5] * adjB[5] + adjB[6] * adjB[6] + adjB[7] * adjB[7] + adjB[8] * adjB[8];
  ...
  inline static double OrthogonalityError(const Eigen::Matrix<double, 9, 1> &a) {
    ...
    return (sq_norm_a1 - 1) * (sq_norm_a1 - 1) + (sq_norm_a2 - 1) * (sq_norm_a2 - 1)
